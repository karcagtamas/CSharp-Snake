...
namespace Snake_v3
{
    class Snake
    {
        ...
        public Snake(int speed, string name)
        {
            this.Speed = speed;
            int k = 10;
            if (Console.WindowWidth == Console.LargestWindowWidth * 4 / 10) k = 4;
            if (Console.WindowWidth == Console.LargestWindowWidth * 4 / 10) this.Level = 0;
            else if (Console.WindowWidth == Console.LargestWindowWidth * 7 / 10) this.Level = 1;
            else this.Level = 2;
            this.Name = name;
            Console.Clear();
            for (int i = 0; i < k; i++)
        ...
        {
            Thread.Sleep(Speed);
            if (point > 50) SuperSnake();
            if (check) { if (IsBitItself() || IsOut()) this.Death(); }
            else if (IsOut()) this.Death();
            this.IsEaten();
        ...
        protected void SuperSnake()
        {
            check = true;
            for (int i = 0; i < SnakeL.Count; i++)
            {
                SnakeL[0].Cc = ConsoleColor.DarkMagenta;
            }
        }
    }
}

    ...
    class Game
    {
        ...
        private void StarterSettings()
        {
            if (!Directory.Exists(@"C:\ProgramData\SnakeV3"))
            {
                ...
                File.AppendAllLines(@"C:\ProgramData\SnakeV3\Profiles\Default\Scores.txt", new List<string> { "0;0;0", "0;0;1","0;0;2" },Encoding.UTF8);
            }
            ...
            for (int i = 0; i < S.Length; i++)
            {
                ...
                for (int j = 0; j < d2.Length; j++)
                {
                    ...
                    p.Levels.Add(int.Parse(d2[j].Split(';')[2]));
                }
                Profiles.Add(p);
            }
        }
        ...
        private void CreateProfile()
        {
            ...
            File.AppendAllLines(@"C:\ProgramData\SnakeV3\Profiles\" + p.Name + @"\Scores.txt", new List<string> { "0;0;0", "0;0;1", "0;0;2" }, Encoding.UTF8);
        ...
        {
            Menu StatMenu = new Menu((Console.WindowWidth / 2) - 4, Console.WindowHeight / 2 - 2, new List<string> { "Nehéz", "Közepes", "Könnyű", "Vissza" });
            int answer = 1;
            bool f = true;
            while (answer > -1 && f)
            {
                answer = StatMenu.Update();
                switch (answer)
                {
                    case 0: this.Level2(); StatMenu = new Menu((Console.WindowWidth / 2) - 4, Console.WindowHeight / 2 - 2, new List<string> { "Nehéz", "Közepes", "Könnyű", "Vissza" }); break;
                    case 1: this.Level1(); StatMenu = new Menu((Console.WindowWidth / 2) - 4, Console.WindowHeight / 2 - 2, new List<string> { "Nehéz", "Közepes", "Könnyű", "Vissza" }); break;
                    case 2: this.Level0(); StatMenu = new Menu((Console.WindowWidth / 2) - 4, Console.WindowHeight / 2 - 2, new List<string> { "Nehéz", "Közepes", "Könnyű", "Vissza" }); break;
                    case 3: f = false; break;
                }
            }
        ...
        private void Level0()
        {
            ...
            {
                Names.Add(i.Name);
                Points.Add(i.MaxScore(0));
                Steps.Add(i.MaxStep(0));
            }
            int db = 1;
        ...
        private void Level1()
        {
            ...
            {
                Names.Add(i.Name);
                Points.Add(i.MaxScore(1));
                Steps.Add(i.MaxStep(1));
            }
            int db = 1;
            for (int i = Points.Max(); i > 0; i--)
            {
        ...
        private void Level2()
        {
            ...
            {
                Names.Add(i.Name);
                Points.Add(i.MaxScore(2));
                Steps.Add(i.MaxStep(2));
            }
            int db = 1;
            for (int i = Points.Max(); i > 0; i--)
            {
        ...
        {
            string[] s1 = { S.Point + ";" + S.Steps + ";" + S.Level};
            ...
            Profiles[ActiveProfile].Levels.Add(S.Level);
        }
    }
}
